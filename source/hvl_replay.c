...
/*
static inline void clr_l( ULONG *src, uint32 longs)
{
  do {
    *src++ = 0;
    longs--;
...
void hvl_set_audio( struct hvl_voice *voice, f32 freqf )
{
  if( voice->vc_TrackOn == 0 )
  {
...
void hvl_play_irq( struct hvl_tune *ht )
{
  ...
  for( i=0; i<ht->ht_Channels; i++ )
    hvl_set_audio( &ht->ht_Voices[i], ht->ht_FreqF );
}
